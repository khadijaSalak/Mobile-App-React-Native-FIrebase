...
var useruid=Firebase.auth().currentUser.uid
 ...
 try{
  ...
  docuse=docuse+1;
   var ref = Firebase.storage().ref().child("docImg1/"+docuse);
...
changeimg(){
  ...
  return Firebase.database().ref('InfoDoctor/'+ useruid).update({
     photo:phot
  }).then(()=>this.reset());}else{
...
reset(){
 ...
 render(){
   return(
     <View style={{alignSelf:'center',alignItems:'center',top:SCREEN_HEIGHT-460}}>
   <Loader isLoading={this.state.isLoading}/>
         {this.state.phot !="" ? <Image source={{uri:this.state.phot}} style={{borderRadius:60,width:120,height:120}}/>:<Image source={require('../assets/profdoc.jpg')} style={{borderWidth:1.5,borderColor:'gray',borderRadius:60,width:120,height:120}}/>}
